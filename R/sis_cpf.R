#' @rdname sis_cpf
#' @title Controlled particle filter for SIS model 
#' @description Runs an controlled particle filter for SIS agent-based model 
...
sis_cpf <- function(model_config, observations, filter_config){
  # start timer
  ...
  for (t in 1:T){
    ...
    if (t < T){
      alpha <- sis_alpha_prob(model_config, X, I) # N x P
      I_support <- infected_support(y[t+2]) # M = length(I_support)
      log_poibin <- dpoibin(I_support, alpha) # M x P
    ...
    # compute controlled weights and ESS 
    log_dmeasurement <- dmeasurement(y[t+1], I) # vector of size P
