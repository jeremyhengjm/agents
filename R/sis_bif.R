#' @rdname sis_bif
#' @title Backward information filter approximation for SIS model 
#' @description Approximate backward information filter for SIS agent-based model 
...
sis_bif <- function(model_config, observations, conditional_expectation = 'sumbin'){
  # start timer
  ...
  # pre-allocate
  log_bif <- matrix(-Inf, nrow = N+1, ncol = T+1)
  
  # backward information filter at the terminal time
  I_current_support <- infected_support(y[T+1])
  log_bif[I_current_support+1, T+1] <- dmeasurement(y[T+1], I_current_support) 
  ...
  for (t in T:1){
    ...
    I_current_support <- infected_support(y[t])
    ...
    log_dmeasurement <- dmeasurement(y[t], I_current_support) 
