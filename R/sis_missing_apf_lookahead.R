#' @rdname sis_missing_apf_lookahead
#' @title Look-ahead approximation of auxiliary particle filter for SIS model with missing data
#' @description Look-ahead approximation of auxiliary particle filter for SIS agent-based model with missing data
...
sis_missing_apf_lookahead <- function(model_config, observations, conditional_expectation = 'sumbin'){
  # start timer
  ...
  for (t in nsteps:1){
    ...
    if (obs_times[t]){
      ...
      current_log_bif <- log_dmeasurement
    } else{
      ...
      current_log_bif <- log_condexp_bif
    }
    log_bif[I_current_support+1, t] <- current_log_bif
  }
